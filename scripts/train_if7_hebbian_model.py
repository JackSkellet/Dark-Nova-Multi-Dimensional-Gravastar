...
def build_record(
    *,
    ...
    metrics = run_if7_hebbian_trained_model(corpus_jsonl=corpus_jsonl, config=config)
    ...
    command = (
        "uv run python scripts/train_if7_hebbian_model.py "
    ...
    record = ExperimentRecord(
        experiment_id=experiment_id,
        hypothesis="if7_hebbian_conditioning_improves_trained_real_corpus_model",
        seed=config.seed,
        command=command,
        metrics=metrics,
        status="completed",
        notes=(
            "Trained IF7 supervised multilabel model on real corpus rows. Compares "
            "a cue-only trained model against a cue-plus-Hebbian-conditioned trained model."
        ),
    ).to_jsonable()
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("results/IF7b_hebbian_trained_model_d5.json"),
    )
    parser.add_argument("--experiment-id", default="IF7b_hebbian_trained_model_d5")
