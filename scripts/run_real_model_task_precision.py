...
from weightlab.real_model_task_precision import evaluate_tiny_gpt2_task_precision
...
def _token_sequences(
    ...
    return torch.randint(0, token_pool_size, (n_sequences, sequence_length), generator=generator)
...
def main() -> None:
    ...
    parser.add_argument("--calibration-sequences", type=int, default=12)
    parser.add_argument("--heldout-sequences", type=int, default=12)
    parser.add_argument("--sequence-length", type=int, default=16)
    parser.add_argument("--token-pool-size", type=int, default=256)
    ...
    calibration = _token_sequences(
        seed=args.seed + 101,
        n_sequences=args.calibration_sequences,
        sequence_length=args.sequence_length,
        token_pool_size=args.token_pool_size,
    )
    heldout = _token_sequences(
        seed=args.seed + 202,
        n_sequences=args.heldout_sequences,
        sequence_length=args.sequence_length,
        token_pool_size=args.token_pool_size,
    )
    metrics = evaluate_tiny_gpt2_task_precision(
        checkpoint_path=args.checkpoint_path,
    ...
    command = (
        "uv run python scripts/run_real_model_task_precision.py "
        ...
        f"--calibration-sequences {args.calibration_sequences} "
        f"--heldout-sequences {args.heldout_sequences} "
        f"--sequence-length {args.sequence_length} "
        f"--token-pool-size {args.token_pool_size} "
    ...
    record = ExperimentRecord(
        experiment_id="E3i_real_open_model_task_precision",
        hypothesis="H3",
        seed=args.seed,
        command=command,
        metrics=metrics,
        notes=(
            "Pinned tiny-gpt2 checkpoint evaluated with a local minimal GPT-2 forward pass. "
            "Measures held-out next-token loss and KL on deterministic token-id sequences; "
            "not a natural-language benchmark."
        ),
    ).to_jsonable()
    args.output_dir.mkdir(parents=True, exist_ok=True)
    write_json(args.output_dir / "E3i_real_open_model_task_precision.json", record)
