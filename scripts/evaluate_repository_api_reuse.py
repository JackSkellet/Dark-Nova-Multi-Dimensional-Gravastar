...
def main() -> None:
    ...
    parser.add_argument("--max-source-rows", type=int, default=50_000)
    parser.add_argument("--max-query-rows", type=int, default=10_000)
    parser.add_argument("--min-text-bytes", type=int, default=80)
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("results/D5_repository_api_reuse_validation.json"),
    )
    parser.add_argument("--experiment-id", default="D5_repository_api_reuse_validation")
    ...
    metrics = evaluate_repository_api_reuse(args.corpus_jsonl, config)
    ...
    command = (
        "uv run python scripts/evaluate_repository_api_reuse.py "
    ...
    record = ExperimentRecord(
        ...
        status="completed",
        notes=(
            "Scores repository-local existing-symbol selection from query rows that mention "
            "source API symbols. This is an API-reuse proxy, not code generation or "
            "executable runtime scoring."
        ),
    ).to_jsonable()
