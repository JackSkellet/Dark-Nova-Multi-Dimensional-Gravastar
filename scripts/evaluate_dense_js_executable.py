...
def main() -> None:
    ...
    parser.add_argument("--prefix-chars", type=int, default=16)
    parser.add_argument("--target-tokens", type=int, default=48)
    ...
    metrics = evaluate_dense_js_executable_checkpoint(
    ...
    record = ExperimentRecord(
        ...
        status="completed",
        notes=(
            "Evaluation-only held-out D4 JavaScript line-completion syntax probe. "
            "Generated completions are checked with node --check; this is not a full "
            "unit-test, repair, or API-reuse benchmark."
        ),
    ).to_jsonable()
