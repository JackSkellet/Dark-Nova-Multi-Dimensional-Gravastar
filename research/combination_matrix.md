...
| `dense_ranked_repair_selection` | Dense checkpoint repair-candidate likelihood ranking | revise | T11c ranks 4/4 passing deterministic edits first in the six-candidate pool; four-program bounded top-k syntax-pool execution repairs 4/4 at top-8; after the counter-zero mutation, the 12-program slice repairs 4/12 by top-8 under dense likelihood. | Free-form dense generation still repairs 0/4; four-program controls beat dense on candidate budget; the 12-program counter-zero run does not show dense beating repair-aware or random controls on program repair rate. | Add learned or failure-localized ranking and require it to beat repair-aware and multiple random controls after candidate coverage improves beyond 4/12. |
| `syntax_preserving_mutation_pool` | Broader syntax-preserving AST mutation pool | revise | Four-program smoke builds 63 syntax-valid candidates and contains passing candidates for all four tasks; the 12-program slice moved from 3/12 to 4/12 after adding final-counter-zero mutation. | The pool is still hand-designed AST mutation scaffolding, and 8/12 programs have no passing selected candidate after the counter-zero addition. | Add another non-oracle mutation family and require the 12-program gate to move beyond 4/12 without worsening same-pool controls. |
| `repair_aware_static_ordering` | Repair-aware static syntax-pool ordering | revise | Four-task smoke repairs 4/4 at top-1; on the 12-program counter-zero run, repair-aware order repairs 3/12 at top-1 and 4/12 by top-8. | Dense likelihood, repair-aware ordering, and seeded random tie at 4/12 by top-8 after the counter-zero mutation, so this is not a robust selector win. | Add failure-localized or learned repair-aware features and require top-1/top-4 gains over deterministic, dense-likelihood, and multiple random controls. |
...
| `glm_5_2_external_baseline_teacher_track` | GLM-5.2 external baseline and teacher track | active | Primary Z.ai/Hugging Face sources report GLM-5.2 long-context and coding benchmark results; Z.ai docs expose relevant API-facing capabilities including 1M context, 128K max output, thinking mode, function calling, context caching, structured output, and project-level engineering use cases; `GLM5_2_public_eval_harness` records the first offline public/synthetic/approved-data harness and smoke task pack; `T11c_dense528_glm_public_smoke` scores the same-budget local dense checkpoint at 0/3. | Published numbers are external, GLM-5.2 is not a same-budget local baseline, teacher data can produce style imitation without held-out executable gains, and no saved GLM-5.2 outputs have been scored on the harness yet. | Score saved GLM-5.2 outputs under explicit prompt, budget, tool, cost, latency, token, and judge metadata; then test one teacher-data slice against a no-teacher control. |
...
- GLM-5.2 primary sources and technical context: <https://z.ai/blog/glm-5.2>, <https://docs.z.ai/guides/llm/glm-5.2>, <https://huggingface.co/blog/zai-org/glm-52-blog>, <https://huggingface.co/zai-org/GLM-5.2>, <https://github.com/zai-org/GLM-5>, <https://arxiv.org/abs/2602.15763v2>, <https://arxiv.org/abs/2606.12370>

## Baseline Categories

GLM-5.2 must stay separated from local same-budget claims:

1. Local same-budget baseline: dense-528 and successors on the same hardware, token budget, dataset, and evaluation.
2. Local practical baseline: best one-workstation model, quantized when useful, with quality plus tokens/sec, VRAM, RAM, and size.
3. External GLM-5.2 baseline: API or separately served model used as external frontier target, oracle, teacher, and benchmark reference.
4. Published GLM-5.2 reference: official or third-party reported numbers, marked as not reproduced unless the same harness is run here.

Remote GLM-5.2 calls may use only public, synthetic, or explicitly approved data. Never send private repositories, private prompts, confidential code, secrets, credentials, customer data, or restricted documents to a remote GLM-5.2 service.
...
3. `dense_ranked_repair_selection` + `syntax_preserving_mutation_pool`: `QuixBugs_T11c_dense528_counter_zero_syntax_controls_12prog` records the first candidate-generation improvement on the 12-program slice. Final-counter-zero mutation increases selected repair coverage from 3/12 to 4/12; dense likelihood reaches 4/12 by top-8, tied with repair-aware static order and seeded random on program rate.
4. `repair_aware_static_ordering` + `syntax_preserving_mutation_pool`: counter-zero mutation gives repair-aware order 3/12 repairs at top-1 and 4/12 by top-8, an early-placement improvement but not a robust selector win because dense/random also reach 4/12 by top-8.
...
8. `execution_feedback_self_refinement` + `syntax_preserving_mutation_pool`: candidate coverage moved from 3/12 to 4/12 with a generic mutation. A feedback loop should improve uncovered failure modes before ranking the same weak pool harder.
...
13. `glm_5_2_external_baseline_teacher_track` + `repobench_repository_completion_gate`: `GLM5_2_public_eval_harness` creates the first offline public/synthetic/approved-data harness with prompt/context/output/cost/latency/token/judge metadata checks. `T11c_dense528_glm_public_smoke` scores the local dense-528 same-budget baseline at 0/3. Next score saved GLM-5.2 outputs on the same tasks.
14. `glm_5_2_external_baseline_teacher_track` + `repository_api_reuse_objective`: generate public API-reuse or repair teacher samples, filter by tests/source-grounding, and train/evaluate against a no-teacher control.
