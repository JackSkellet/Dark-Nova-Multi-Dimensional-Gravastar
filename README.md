...
Generated metrics are written to `results/manifest.json`, one JSON file per experiment, and `results/summary.csv`. The current manifest preserves explicit standalone runs and contains 129 records, including real-training, split-correct evaluation, held-out functional probes, trained-checkpoint quantization records, the completed exploratory D5 corpus materialization and audit, bounded tokenizer-efficiency comparisons, and the first T12 second-seed dense-528 checkpoint evaluation. The current assessment records T11 frontier expansion without Pareto dominance: dense-528 T11c leads validation loss, storage, VRAM, gradient stability, and throughput, while residual-adapter T11b keeps a small final-test-loss edge. T12 remains incomplete until the matched residual-adapter seed456 run is trained and evaluated.
