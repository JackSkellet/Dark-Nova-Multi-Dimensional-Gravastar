...
Generated metrics are written to `results/manifest.json`, one JSON file per experiment, and `results/summary.csv`. The current manifest preserves explicit standalone runs and contains 121 records, including real-training, split-correct evaluation, held-out functional probes, trained-checkpoint quantization records, and the first bounded tokenizer-efficiency probe. The current assessment records T11 frontier expansion without Pareto dominance: dense-528 T11c leads validation loss, storage, VRAM, gradient stability, and throughput, while residual-adapter T11b keeps a small final-test-loss edge.
