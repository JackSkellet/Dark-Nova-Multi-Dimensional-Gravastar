...
Generated metrics are written to `results/manifest.json`, one JSON file per experiment, and `results/summary.csv`. The current manifest preserves explicit standalone runs and contains 135 records, including real-training, split-correct evaluation, held-out functional probes, trained-checkpoint quantization records, the completed exploratory D5 corpus materialization and audit, bounded tokenizer-efficiency comparisons, and the T12 second-seed dense-528 versus residual-adapter-528 comparison. The current assessment records T11/T12 frontier expansion without Pareto dominance: dense-528 leads mean validation loss, storage, VRAM, stability, and throughput, while residual-adapter-528 keeps a mean final-test-loss edge. T12 recommends a third seed because the validation margin is small and the validation winner changes by seed.
